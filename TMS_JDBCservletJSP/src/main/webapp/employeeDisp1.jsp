<%@page import="com.surya.model.Employee"%>
...
<body>
	...
	%>
    <div class="container">
	<h1>Assigned Task By Manager</h1>
	<!-- 
	...
	<form method="get" action="employeeServlet">
		<input type="hidden" name="sbt" value="getAll"> <%-- <input
	...
	<table border="1">
		...
		List<Employee> dispEmpTasks = (List<Employee>) request.getAttribute("dispEmpTask");
		System.out.println("jsp::viewAssignTask----------" + dispEmpTasks);
		if (dispEmpTasks != null ) {
		//for (Task task : tasks) {
			//if(email.equals((String)task.getTaskAssignManager())){
			for(Employee dispEmpTask: dispEmpTasks){
				if(dispEmpTask.getEmpUserEmail().equals(userEmail)){
				%>
		<tr>
			<td><%=dispEmpTask.getEmpUserEmail() %>
			<td><%=dispEmpTask.getTaskId()%></td>
			<td><%=dispEmpTask.getTaskName()%></td>
			<td><%=dispEmpTask.getTaskDescription()%></td>
			<td><%=dispEmpTask.getTaskAssignManager() %>
			<td><%=dispEmpTask.getTaskAssignDate()%></td>
			<td><%=dispEmpTask.getTaskDueDate()%></td>
			<td><%=dispEmpTask.getTaskStatus()%></td>
 			<td><a href="editEmpStatus.jsp?taskId=<%=dispEmpTask.getTaskId()%>">Edit</a> 
		...
		<%
			...
			//else{%>
				<!-- <tr>
			<td colspan="8">No Assigned Task Yet...</td>
		</tr> -->
			<%// }
			//}
